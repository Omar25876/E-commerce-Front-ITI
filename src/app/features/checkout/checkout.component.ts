...
export class CheckoutComponent implements OnInit, AfterViewInit {
  @ViewChild('checkoutWrapper') checkoutWrapper!: ElementRef;
 ...
 user: any;
  ...
  ngAfterViewInit(): void {
    gsap.from(this.checkoutWrapper.nativeElement, {
      duration: 0.8,
      y: 50,
      opacity: 0,
      ease: 'power3.out',
  ...
  navigateWithAnimation(path: string): void {
    gsap.to(this.checkoutWrapper.nativeElement, {
      duration: 0.5,
      y: -30,
      opacity: 0,
      ease: 'power2.inOut',
      onComplete: () => { this.router.navigateByUrl(path); },
    });
  }
